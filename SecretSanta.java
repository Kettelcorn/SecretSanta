...
public class SecretSanta {
   ...
   public static void main(String[] args) throws FileNotFoundException {
      ...
      repeatYear(size, subGroups, rand, output);
   }
   
   // tells user about what the program will do
   public static void intro() {
     String introMessage = "This program will generate who everyone" + 
                           "will be giving a gift to for Secret Santa this year!";
   ...
   // previous year.
   public static void repeatYear(int size, int subGroups, Random rand, PrintStream output) {
      ...
      for (int i = 0; i < subGroups; i++) {
         ...
         for (int x = 0; x < number; x++) {
            names[count] = JOptionPane.showInputDialog(null, "Person #" + (x + 1) + ":",
                           TITLE,JOptionPane.QUESTION_MESSAGE); 
            gaveTo[count] = JOptionPane.showInputDialog(null, "Who did this person give a gift to last year?",
                            TITLE, JOptionPane.QUESTION_MESSAGE);
            count++;    
         }
         System.out.println();
      }
      calculatePartners(subGroups, names, rand, gaveTo, givingTo, groupSize, output);
   }
   ...
   public static void calculatePartners(int subGroups, String[] names, Random rand, 
                        ...
                        int[] groupSize, PrintStream output) {
      int difference = 0;
      for (int i = 0; i < subGroups; i++) {
